...
export const updateMeeting = async (
  ...
  const response = await api.patch<Meeting>(`/api/meetings/${meetingId}`, data);
  return response.data;
};
...
export const getMeetingReminders = async (meetingId: number): Promise<Reminder[]> => {
  const response = await api.get<Reminder[]>(`/api/meetings/${meetingId}/reminders`);
  return response.data;
};
...
export const addMeetingReminder = async (
  ...
  const response = await api.post<Reminder>(`/api/meetings/${meetingId}/reminders`, data);
  return response.data;
};
